...
/**
 * The empty state has to be scoped to workshops and classes, and it has to
 * mention the exhibitions — otherwise a page says "nothing is scheduled" in the
 * same week an exhibition opens, which is both wrong and easy to disprove.
 */
function nothingUpcomingHtml() {
  const soon = [...exhibitions.upcoming, ...exhibitions.current].filter((ex) => ex.start && ex.end);

  const exhibitionLine = soon.length
    ? `<p>Exhibitions are still on: ${soon
        .map(
          (ex) =>
            `<a href="{{base}}${exhibitionPath(ex)}"><em>${esc(ex.title)}</em></a>${
              ex.artist ? ` by ${esc(ex.artist)}` : ''
            }, ${esc(exhibitionDates(ex))}`
        )
        .join('; ')}.</p>`
    : '';

  return `<p>No workshops or classes are scheduled at the moment. New ones are announced by email
      first &mdash; <a href="{{base}}subscribe/">join the mailing list</a> &mdash; and tickets go on sale on
      <a href="${site.external.eventbrite}" target="_blank" rel="noopener">Eventbrite<span class="visually-hidden"> (opens in a new tab)</span></a>.</p>
      ${exhibitionLine}`;
}

// Called at render time, not module load: `exhibitions` is defined further down.
...
function render(template, page) {
  ...
  return template
    ...
    .replace(/\{\{eventsUpcomingShort\}\}/g, () =>
      eventList(upcomingEvents, { limit: 4, kind: 'upcoming', emptyHtml: nothingUpcomingHtml() })
    )
...
function build() {
  ...
  // of them.
  /* Exhibitions run for months, so they go to the calendar as date ranges
     rather than as single-day events. Only ones with both a start and an end
     can be placed on a grid; an exhibition with no published dates is left off
     rather than given invented ones, and the permanent collection is not a
     run at all — it is simply always there. */
  const calendarRuns = exhibitionData.temporary
    .filter((ex) => ex.start && ex.end)
    .map((ex) => ({
      slug: ex.slug,
      title: ex.title,
      artist: ex.artist || '',
  ...
  const dataFile = `/* Generated by build.js — edit src/events.json, src/exhibitions.json or src/site.config.js instead. */
window.MEDICI = ${JSON.stringify(
    { events: eventData.events, exhibitions: calendarRuns, hours: site.hours },
    null,
    2
  )};
`;
